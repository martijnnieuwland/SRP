...
class LastrowidTest(fixtures.TablesTest):
    ...
    def test_last_inserted_id(self, connection):
        ...
        pk = connection.scalar(select(self.tables.autoinc_pk.c.srp_user))
...
class InsertBehaviorTest(fixtures.TablesTest):
    ...
    def test_empty_insert(self, connection):
        ...
        r = connection.execute(
            self.tables.autoinc_pk.select().where(
                self.tables.autoinc_pk.c.srp_user != None
            )
        )
        eq_(len(r.all()), 1)
    ...
    def test_insert_from_select(self, connection):
        ...
        connection.execute(
            ...
            .from_select(
                ("id", "data"),
                select(table.c.srp_user + 5, table.c.data).where(
    ...
    def test_insert_from_select_with_defaults(self, connection):
        ...
        eq_(
            connection.execute(
                select(table).order_by(table.c.data, table.c.srp_user)
            ).fetchall(),
            [
                (1, "data1", 5, 4),
                (2, "data2", 5, 4),
...
class ReturningTest(fixtures.TablesTest):
    ...
    def test_explicit_returning_pk_autocommit(self, connection):
        ...
        r = connection.execute(
            table.insert().returning(table.c.srp_user), dict(data="some data")
        )
        pk = r.first()[0]
        fetched_pk = connection.scalar(select(table.c.srp_user))
        eq_(fetched_pk, pk)
