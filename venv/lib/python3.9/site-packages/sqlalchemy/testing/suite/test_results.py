...
class RowFetchTest(fixtures.TablesTest):
    ...
    def test_via_attr(self, connection):
        row = connection.execute(
            self.tables.plain_pk.select().order_by(self.tables.plain_pk.c.srp_user)
        ).first()
    ...
    def test_via_col_object(self, connection):
        ...
        eq_(row._mapping[self.tables.plain_pk.c.srp_user], 1)
    ...
    def test_row_with_dupe_names(self, connection):
        result = connection.execute(
            ...
            ).order_by(self.tables.plain_pk.c.srp_user)
        )
        row = result.first()
    ...
    def test_row_w_scalar_select(self, connection):
        ...
        s2 = select(datetable.c.srp_user, s.label("somelabel"))
...
class ServerSideCursorsTest(
    ...
    def test_roundtrip_fetchall(self, metadata):
        ...
        with engine.begin() as connection:
            ...
            eq_(
                connection.execute(
                    test_table.select().order_by(test_table.c.srp_user)
                ).fetchall(),
                [(1, "data1"), (2, "data2")],
            )
            connection.execute(
                test_table.update()
                .where(test_table.c.srp_user == 2)
