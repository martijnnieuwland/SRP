...
class CTETest(fixtures.TablesTest):
    ...
    def test_select_recursive_round_trip(self, connection):
        ...
        # can't be UNION
        cte = cte.union_all(
            select(st1).where(st1.c.srp_user == cte_alias.c.parent_id)
        )
        result = connection.execute(
            select(cte.c.data)
            .where(cte.c.data != "d2")
    ...
    def test_insert_from_select_round_trip(self, connection):
        ...
        eq_(
            connection.execute(
                select(some_other_table).order_by(some_other_table.c.srp_user)
            ).fetchall(),
    ...
    def test_delete_scalar_subq_round_trip(self, connection):
        ...
        connection.execute(
            some_other_table.delete().where(
                ...
                == select(cte.c.data)
                .where(cte.c.srp_user == some_other_table.c.srp_user)
                .scalar_subquery()
            )
        )
        eq_(
